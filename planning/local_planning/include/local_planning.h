...
class RelativeInfo
{
  public:
	...
	int iBack;
	int iGlobalPath;
	yunle_msgs::WayPoint perp_point;
...
class TrajectoryGen
{
  public:
	...
	yunle_msgs::WayPoint m_CurrentPos;
	...
	std::vector<yunle_msgs::WayPoint> m_temp_path;
	std::vector<yunle_msgs::WayPoint> m_GlobalPath;
	std::vector<yunle_msgs::WayPoint> m_GlobalPathSection;

	std::vector<std::vector<yunle_msgs::WayPoint> > m_RollOuts;
	...
	//Helper Functions
	double CalcAngleAndCost(std::vector<yunle_msgs::WayPoint> &path);
	int ExtractPartFromPointToDistanceDirectionFast(const std::vector<yunle_msgs::WayPoint> &originalPath, 
													 const yunle_msgs::WayPoint &pos, 
													 std::vector<yunle_msgs::WayPoint> &extractedPath);
	int GetClosestNextPointIndexDirectionFast(const std::vector<yunle_msgs::WayPoint> &trajectory, 
											  const yunle_msgs::WayPoint &p, 
											  const int &prevIndex);
	bool CompareTrajectories(const std::vector<yunle_msgs::WayPoint> &path1, const std::vector<yunle_msgs::WayPoint> &path2);
	void FixPathDensity(std::vector<yunle_msgs::WayPoint> &path, const double &distanceDensity);
	void GenerateRunoffTrajectory(const std::vector<yunle_msgs::WayPoint> &referencePath, 
								  const yunle_msgs::WayPoint &carPos,
								  const double &speed, 
								  std::vector<std::vector<yunle_msgs::WayPoint> > &rollOutPath);
	void CalculateRollInTrajectories(const yunle_msgs::WayPoint &carPos, 
									 const double &speed, 
									 const std::vector<yunle_msgs::WayPoint> &originalCenter, 
									 int &start_index,
									 int &end_index, 
									 std::vector<double> &end_laterals, 
									 std::vector<std::vector<yunle_msgs::WayPoint> > &rollInPaths);

	void SmoothPath(std::vector<yunle_msgs::WayPoint> &path, 
					double weight_data, 
					double weight_smooth, 
					double tolerance);

	bool GetRelativeInfo(const std::vector<yunle_msgs::WayPoint> &trajectory, 
						 const yunle_msgs::WayPoint &p, 
						 RelativeInfo &info, 
						 const int &prevIndex);

	void TrajectoriesToMarkers(const std::vector<std::vector<yunle_msgs::WayPoint> > &paths, 
							   visualization_msgs::MarkerArray &markerArray,
							   const TrajectoryCost &tc);
	void TrajectoriesSpeedToMarkers(const std::vector<std::vector<yunle_msgs::WayPoint> > &paths, 
							   visualization_msgs::MarkerArray &markerArray,
							   const TrajectoryCost &tc);
	...
	std::vector<TrajectoryCost> CalculatePriorityCosts(const std::vector<std::vector<yunle_msgs::WayPoint> > &laneRollOuts);
	...
	bool detectCollision(const yunle_msgs::WayPoint &wp);
	...
	TrajectoryCost DoOneStep(const std::vector<std::vector<yunle_msgs::WayPoint> > &rollOuts, 
