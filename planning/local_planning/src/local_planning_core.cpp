...
void TrajectoryGen::initROS()
{
	...
	pub_LocalTrajectoriesSpeed = nh_.advertise<geometry_msgs::Twist>("current_velocity", 5);
	...
	sub_GlobalPlannerPaths = nh_.subscribe("/vis_global_path", 1, &TrajectoryGen::callbackGetGlobalPlannerPath, this);

	sub_ObstacleMap = nh_.subscribe("/costmap", 10, &TrajectoryGen::callbackGetObstaclesMap, this);
}

double TrajectoryGen::CalcAngleAndCost(std::vector<yunle_msgs::WayPoint> &path)
...
bool TrajectoryGen::CompareTrajectories(const std::vector<yunle_msgs::WayPoint> &path1, const std::vector<yunle_msgs::WayPoint> &path2)
{ //比较两条路径是否有区别
...
int TrajectoryGen::GetClosestNextPointIndexDirectionFast(const std::vector<yunle_msgs::WayPoint> &trajectory, const yunle_msgs::WayPoint &p, const int &prevIndex)
 ...
 */
int TrajectoryGen::ExtractPartFromPointToDistanceDirectionFast(const std::vector<yunle_msgs::WayPoint> &originalPath,
																															 const yunle_msgs::WayPoint &pos,
																															 std::vector<yunle_msgs::WayPoint> &extractedPath)
{ //提取离点最近的部分路径
	...
	std::vector<yunle_msgs::WayPoint> end_interpolation;
	...
	yunle_msgs::WayPoint start = end_interpolation.at(0);
	yunle_msgs::WayPoint end = end_interpolation.at(end_interpolation.size() - 1);
	for (int i = 0; i < end_interpolation.size(); i++)
	{
		...
		if (i > 0)
		{
			end_interpolation.at(i).vel_x = std::max(0.5, sqrt(pow(end_interpolation.at(i - 1).vel_x, 2) - 2 * deceleration * utility.getPlaneDistance(end_interpolation.at(i - 1).ps.pose.position, end_interpolation.at(i).ps.pose.position)));
...
void TrajectoryGen::FixPathDensity(std::vector<yunle_msgs::WayPoint> &path, const double &distanceDensity)
{ //修正path中每个点的间隔
	...
	double remaining = 0;
	int nPoints = 0;
	std::vector<yunle_msgs::WayPoint> fixedPath;
	...
	for (unsigned int si = 0, ei = 1; ei < path.size();)
	{
		...
		{
			yunle_msgs::WayPoint pm = path.at(si);
...
void TrajectoryGen::GenerateRunoffTrajectory(const std::vector<yunle_msgs::WayPoint> &referencePath,
																						 const yunle_msgs::WayPoint &carPos,
																						 const double &speed,
																						 std::vector<std::vector<yunle_msgs::WayPoint>> &rollOutPaths)
{
	...
	std::vector<std::vector<yunle_msgs::WayPoint>> temp_rollOutPath;
...
void TrajectoryGen::CalculateRollInTrajectories(const yunle_msgs::WayPoint &carPos,
																								const double &speed,
																								const std::vector<yunle_msgs::WayPoint> &CenterPath,
																								int &start_index,
																								int &end_index,
																								std::vector<double> &end_laterals,
																								std::vector<std::vector<yunle_msgs::WayPoint>> &rollInPaths)
{
	yunle_msgs::WayPoint wp;
	...
	for (int i = 0; i <= m_PlanningParams.rollOutNumber; i++)
	{
		...
		rollInPaths.push_back(std::vector<yunle_msgs::WayPoint>());
		inc_list_inc.push_back(0);
	}

	std::vector<std::vector<yunle_msgs::WayPoint>> execluded_from_smoothing;
	for (unsigned int i = 0; i <= m_PlanningParams.rollOutNumber; i++)
		execluded_from_smoothing.push_back(std::vector<yunle_msgs::WayPoint>());
...
void TrajectoryGen::SmoothPath(std::vector<yunle_msgs::WayPoint> &path,
															 double weight_data,
															 ...
															 double tolerance)
{
	...
	const std::vector<yunle_msgs::WayPoint> &path_in = path;
	std::vector<yunle_msgs::WayPoint> smoothPath_out = path_in;
...
bool TrajectoryGen::GetRelativeInfo(const std::vector<yunle_msgs::WayPoint> &trajectory,
																		const yunle_msgs::WayPoint &wp,
																		RelativeInfo &info,
																		const int &prevIndex)
{
	...
	yunle_msgs::WayPoint wp0; //最近的前一点
	yunle_msgs::WayPoint wp1; //后一点
	if (trajectory.size() == 2)
	{
		wp0 = trajectory.at(0);
	...
	yunle_msgs::WayPoint prevWP = wp0;
...
TrajectoryCost TrajectoryGen::DoOneStep(const std::vector<std::vector<yunle_msgs::WayPoint>> &rollOuts,
																				const std::vector<yunle_msgs::WayPoint> &totalPaths,
																				const yunle_msgs::WayPoint &currState,
																				const int &currIndex,
																				const double &m_Steer)
{
	...
	if (smallestIndex == -1 && m_PrevCostIndex < (int)m_TrajectoryCosts.size())
	{
		...
		if (smallestDistance < 1.0)
			...
			{
				m_RollOuts.at(currIndex).at(p).vel_x = 0.5;
			}
	}
...
std::vector<TrajectoryCost> TrajectoryGen::CalculatePriorityCosts(const std::vector<std::vector<yunle_msgs::WayPoint>> &laneRollOuts)
{ //计算各支路cost
...
bool TrajectoryGen::detectCollision(const yunle_msgs::WayPoint &wp)
{
...
void TrajectoryGen::CalculateObstacleCosts(std::vector<TrajectoryCost> &trajectoryCosts,
																					 const std::vector<std::vector<yunle_msgs::WayPoint>> &rollOuts,
																					 const yunle_msgs::WayPoint &m_CurrentPos)
{ //计算变道所需的cost
...
void TrajectoryGen::MainLoop()
{
	...
	while (ros::ok())
	{
		...
		if (m_GlobalPathSection.size() > 0)
		{
			...
			printf("choose no.%d path", tc.index);
			...
			else
			{
				...
				pub_twist.linear.x = m_RollOuts.at(tc.index).at(1).vel_x;
				printf("vel = %f ,angle1 = %f", m_RollOuts.at(tc.index).at(1).vel_x, utility.rad2deg(fabs(m_RollOuts.at(tc.index).at(1).angle - m_RollOuts.at(tc.index).at(0).angle)));
			}
		}
