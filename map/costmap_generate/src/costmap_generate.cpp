...
void UpdateCostmap::initROS()
{
    ros::Duration(1.0).sleep();
    private_nh_.param<std::string>("frame_map", param_frame_map_, "map");
    private_nh_.param<std::string>("frame_laser", param_frame_laser_, "velodyne");
    private_nh_.param<std::string>("frame_base", param_frame_base_, "base_link");
    private_nh_.param<double>("max_range", param_max_range_, 20.);
    private_nh_.param<double>("angle_resolution", param_angle_resolution_, 0.2);
    private_nh_.param<int>("decay", param_decay_, 10);
    ...
    fake_scan_t.resize(fake_num_rays_);
    fake_scan_.resize(fake_num_rays_);
    for (int i = 0; i < fake_scan_t.size(); i++) {
        fake_scan_t[i].rad = i * fake_angle_resolution_;
        fake_scan_t[i].dist = param_max_range_;
        fake_scan_t[i].x = param_max_range_ * std::cos(fake_scan_t[i].rad);
        fake_scan_t[i].y = param_max_range_ * std::sin(fake_scan_t[i].rad);
    }
    fake_scan_.insert(fake_scan_.end(), fake_scan_t.begin(), fake_scan_t.end());
    ROS_INFO("fake_scan_t.size %d; fake_scan.size %d", fake_scan_t.size(), fake_scan_.size());
    ROS_INFO("fake_num_rays: %d", fake_num_rays_);
    msg_update_map_.header.frame_id = param_frame_map_;

    pub_costmap_ = nh_.advertise<nav_msgs::OccupancyGrid>("costmap", 1);
    pub_viz_ = nh_.advertise<visualization_msgs::Marker>("viz_scan", 1);

    dynamic_reconfigure::Server<costmap_generate::GenCostmapConfig> cfg_server;
    dynamic_reconfigure::Server<costmap_generate::GenCostmapConfig>::CallbackType cfg_cb = boost::bind(&UpdateCostmap::cfgCB, this, _1, _2);
...
{
    sub_obstacle_pc_ = nh_.subscribe<sensor_msgs::PointCloud2>("/ray_filter/velodyne_points_costmap", 1, boost::bind(&UpdateCostmap::obstacleCB, this, _1));
    sub_pose_ = nh_.subscribe<geometry_msgs::PoseStamped>("current_pose", 1, boost::bind(&UpdateCostmap::poseCB, this, _1));
    sub_map_ = nh_.subscribe<nav_msgs::OccupancyGrid>("global_costmap", 1, boost::bind(&UpdateCostmap::mapCB, this, _1));
...
void UpdateCostmap::obstacleCB(const sensor_msgs::PointCloud2ConstPtr &msg)
{
    ...
    {
        s.x = int((s.x + pose_laser.position.x - msg_map.info.origin.position.x) / msg_map.info.resolution);
        s.x = s.x < 0 ? 0 : (s.x >= msg_map.info.width ? msg_map.info.width - 1 : s.x);
        s.y = int((s.y + pose_laser.position.y - msg_map.info.origin.position.y) / msg_map.info.resolution);
        s.y = s.y < 0 ? 0 : (s.y >= msg_map.info.height ? msg_map.info.height - 1 : s.y);
    }
    ...
    int rad_index = 0;
    // 雷达在栅格中的位置
    int l_x = int((pose_laser.position.x - msg_map.info.origin.position.x) / msg_map.info.resolution);
    int l_y = int((pose_laser.position.y - msg_map.info.origin.position.y) / msg_map.info.resolution);
    if (!(l_x >= 0 && l_x < msg_map.info.width && l_y >= 0 && l_y < msg_map.info.height))
    {
    ...
    {
        // 观测点在地图中的位置
        p_x = int((pc_m->points[i].x - msg_map.info.origin.position.x) / msg_map.info.resolution);
        p_y = int((pc_m->points[i].y - msg_map.info.origin.position.y) / msg_map.info.resolution);
        ...
        if (!(p_x >= 0 && p_x < msg_map.info.width && p_y >= 0 && p_y < msg_map.info.height))
        {
            // 忽略地图外的点
            continue;
        }
    ...
    for (int i = 0; i < fake_scan_.size(); ++i)
    {
        ...
        tmp_p.x = msg_map.info.origin.position.x + l_x * msg_map.info.resolution;
        tmp_p.y = msg_map.info.origin.position.y + l_y * msg_map.info.resolution;
        msg_marker.points.push_back(tmp_p);
        tmp_p.x = msg_map.info.origin.position.x + fake_scan_[i].x * msg_map.info.resolution;
        tmp_p.y = msg_map.info.origin.position.y + fake_scan_[i].y * msg_map.info.resolution;
        msg_marker.points.push_back(tmp_p);
        std::vector<std::pair<int, int>> points;
        getLine(l_x, l_y, fake_scan_[i].x, fake_scan_[i].y, points);
        for (auto &p : points)
        {
            if (msg_update_map_.data[p.second * msg_map.info.width + p.first] >= param_decay_)
            {
                msg_update_map_.data[p.second * msg_map.info.width + p.first] -= param_decay_;
            }
            else
            {
                msg_update_map_.data[p.second * msg_map.info.width + p.first] = 0;
            }
        }
        if (fake_scan_[i].dist < param_max_range_)
        {
            msg_update_map_.data[points[points.size() - 1].second * msg_map.info.width + points[points.size() - 1].first] = 100;
        }
    }
    pthread_mutex_unlock(&mutex_);
    msg_update_map_.header.stamp = msg->header.stamp;
    pub_costmap_.publish(msg_update_map_);
...
void UpdateCostmap::mapCB(const nav_msgs::OccupancyGridConstPtr &msg)
{
    ...
    if (msg->info.height != msg_map.info.height || msg->info.width != msg_map.info.height)
    {
        pthread_mutex_lock(&mutex_);
        msg_map = *msg;
        msg_update_map_.data.clear();
        msg_update_map_.data.insert(msg_update_map_.data.end(), msg->data.begin(), msg->data.end());
        msg_update_map_.info = msg->info;
        // TODO: build lookup table
        pthread_mutex_unlock(&mutex_);
        map_set_ = true;
        max_range_pixel_ = int(param_max_range_ / msg_map.info.resolution);
        ROS_INFO_NAMED(NAME_, "new global map set.");
    }
}

void UpdateCostmap::cfgCB(const costmap_generate::GenCostmapConfig& config, uint32_t level)
{
