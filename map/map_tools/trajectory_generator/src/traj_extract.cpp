...
bool smartcar::tools::Traj_Extractor::create_trajtory(smartcar::utils::traj_info &traj_info)
{
...
/*
 *  weight_data = 0.47, weight_smooth = 0.2, tolerance = 0.2, 非常重要
 */
...
bool smartcar::tools::Traj_Extractor::on_create_trajectory(smartcar::utils::traj_info &type)
{
    if (current_path.poses.empty())
    {
    ...
    else if (type.type == smartcar::utils::traj_type::LANE)
    {
    ...
    else if (smartcar::utils::traj_type::CROSS)
    {
...
bool smartcar::tools::Traj_Extractor::add_text(const smartcar::utils::traj_info &type, const geometry_msgs::Pose &pose)
{
    ...
    if (type.type == smartcar::utils::traj_type::LANE) // lane -> blue // color range: [0,1]
    {
        text.scale.z = 1.0;
        text.color.g = 1;
        text.color.b = 1;
...
bool smartcar::tools::Traj_Extractor::add_line(const smartcar::utils::traj_info &type)
{
    static int cnt = 1;
    for (int j = 0; j < current_path.poses.size() - 1; j++)
    {
        ...
        if (type.type == smartcar::utils::traj_type::LANE)
        {
                                                 ...
                                                 const int id)
{
    ...
    file_name << _output_folder << path_type << "_" << id << ".lane";
    std::ofstream f;
