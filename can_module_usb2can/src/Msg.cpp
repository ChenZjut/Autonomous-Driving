...
{
    writeInt(canMsg.Data, SCU_DRIVE_MODE_REQ_OFFSET, SCU_DRIVE_MODE_REQ_LENGTH, driveMode);  //自动驾驶模式，与档位有区别，始6长2
}

void SendMsg::setAccMode(int v)
{
    writeInt(canMsg.Data, SCU_ACC_MODE_OFFSET, SCU_ACC_MODE_LENGTH, v);  //加速模式，0,1,2,3,0无加速123对应三挡加速，始2长2
}

void SendMsg::setBrakeMode(int v)
{
    writeInt(canMsg.Data, SCU_BRAKE_MODE_OFFSET, SCU_BRAKE_MODE_LENGTH, v);//0123四种制动模式，0不影响车辆状态，始4长2
}

void SendMsg::setSpeed(double v)
{
    int speed = (int)lround(v * 10);
    writeInt(canMsg.Data, SCU_TARGET_SPEED_OFFSET, SCU_TARGET_SPEED_LENGTH, speed);  //设置目标车速，始24长9
}
...
{
    writeInt(canMsg.Data, SCU_SHIFT_LEVEL_OFFSET, SCU_SHIFT_LEVEL_LENGTH, shift_level);//123分别对应前进空挡后退，始0长2
}

void SendMsg::setGradient(double v)
{
    int iv = v * 2; // 因为can里面gradient的单位是0.5度
    writeInt(canMsg.Data, SCU_GRADIENT_OFFSET, SCU_GRADIENT_LENGTH, iv);//道路倾斜度设置 ，始34长6
}
...
void SendMsg::setEBrake(bool need)
{
    ...
    writeInt(canMsg.Data, SCU_EBRAKE_OFFSET, SCU_EBRAKE_LENGTH, v);//紧急制动，始33长1
}
...
}//将上层发布的ROS消息传递给can，理论上，上位机只要向/ecu输送消息即可

VCI_CAN_OBJ SendMsg::getMessage()
{
    this->rosMsg2canMsg();
    VCI_CAN_OBJ return_msg = this->canMsg;//返回发送给CAN的消息
    return return_msg;
}

// -*-*-*-*- Received canMsg: vehicle_status -*-*-*-*-以下为上位机接受can信息注意  注意！！上方是SCU，下方是CCU

int VehicleStatusMsg::shiftLevel()
{
    return readAsInt(this->canMsg.Data, CCU_SHIFT_LEVEL_OFFSET, CCU_SHIFT_LEVEL_LENGTH); //获取车辆当前信息，此处为档位信息
}
...
{
    int dir = readBit(this->canMsg.Data, CCU_WHEEL_DIRECTION_OFFSET);// 实际方向盘朝向 0 左 1右
    if (dir == 0)
        return left;
    return right;
}
